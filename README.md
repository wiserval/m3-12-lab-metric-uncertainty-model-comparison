...
- Compute bootstrap confidence intervals for classification metrics (F1, precision, recall).
- Run a paired permutation test to determine whether two models perform significantly differently.
- Interpret confidence interval overlap to assess whether models are meaningfully different.
...
3. Read through all three tasks before you start so you know what outputs to collect along the way.
...
4. Print a classification report for each model.

Deliverable: trained models, test-set predictions, and a printed comparison table showing F1, precision, and recall for each model.

### Task 2: Bootstrap Confidence Intervals

For each model, compute 95 % bootstrap confidence intervals for F1-score, precision, and recall:

1. Write a reusable function `bootstrap_metric(y_true, y_pred, metric_fn, n_boot=2000, seed=42)` that returns the point estimate and the 2.5th/97.5th percentile interval.
2. Apply the function to each model and each metric.
3. Visualize the results: create a grouped bar chart (or dot-and-whisker plot) where each model has bars for F1, precision, and recall, with error bars showing the 95 % CI.
...
- [ ] Bootstrap 95 % CIs computed for F1, precision, and recall with a comparison visualization (Task 2).
